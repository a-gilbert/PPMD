...
class GlobalParams(dict):
	def __init__(self, infile, comm, rank):
		super(GlobalParams, self).__init__()
		self._empty_init()
		if rank == 0:
			in_file = open(user_in, 'r')
	...
	def interpret(self, line):
		...
		if l[0] == "#":
			...
			for i in range(2, len(l), 2):
				if l[i] == "Depth":
					self[GPK.RCRIT].append([RefineCriteria.MAX_DEPTH, int(l[i+1])])
				elif l[i] == "Density":
					self[GPK.RCRIT].append([RefineCriteria.MAX_DENS, float(l[i+1])])
				elif l[i] == "Length":
					self[GPK.RCRIT].append([RefineCriteria.MAX_LENGTH, float(l[i+1])])
		elif l[0] == "NSteps":
