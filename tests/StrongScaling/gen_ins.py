...
for i in range(1, max_nranks+1):
    for k in range(ntrials):
        ...
        for j in range(len(standard_template)):
            ...
            if j == 9:
                l = len(line)
                line = line[0:l-1] + " uniform_random_2types_%spart.hdf5\n" % npart
            if j == 13:
                line = line.split()
...
rscript = open('run_strong_scaling.sh', 'w')

l = len(rscript_template[0])
rscript.write(rscript_template[0][0:l-1] + ' ' + npart + '\n')
for t in ins:
    if use_cprofiler:
